...
import type { MovanProjectExtension } from "./project-manifests/movan";

// AutoDev 범용화 Phase A Task A5 — CoreState/MOVAN 전용 상태 분리 검증.
//
// 이 파일은 오직 다음 5가지만 증명한다(§ A5 "9. 필수 테스트" A~E). F~J(A1~A4 회귀,
// autodev/orchestrator/checkpoint 전체 회귀)는 기존 test:* 스크립트들을 그대로 재실행해서
// 증명하고, K/L(tsc/build)/M(실제 state hash 불변)은 이 파일이 아니라 Task 실행 절차에서
// 별도로 확인한다.
//
// A) 실제 MOVAN state JSON을 새 타입(ProjectState/CoreState)으로 정상 load할 수 있다.
// B) 기존 MOVAN 전용 필드가 하나도 유실되지 않는다.
// C) CoreState 필드만 가진 작은 Fixture project state로 AutoDev 핵심 next-task 판단이 가능하다.
// D) 그 Fixture state가 MOVAN 전용 필드를 요구하지 않는다(타입 레벨 — 아래 fixture는
//    `CoreState` 타입 그대로 선언되어 있고, MOVAN 필드를 하나도 갖지 않는다).
// E) state load/save round-trip 후 알 수 없는 project-specific 필드가 보존된다.
//
// 실제 automation/config/project-state.json은 read-only 증거로만 쓴다 — 어떤 시나리오도
// 이 경로에 쓰지 않는다(끝에서 실행 전/후 내용 비교로 증명, § 요구사항 project-state 테스트
// 격리).
...
// ---------------------------------------------------------------------------
// A) / B) 실제 MOVAN state JSON을 그대로 복사한 temp 파일을 loadState()로 읽어, 알려진
// MOVAN 전용 필드(project-manifests/movan.ts의 MovanProjectExtension)가 하나도 유실되지
// 않았는지 원본 JSON과 값 단위로 대조한다.
// ---------------------------------------------------------------------------
function scenarioLoadRealMovanStateAndCheckNoFieldLoss(): void {
  const realRaw = readFileSync(DEFAULT_STATE_PATH, "utf-8");
  const original = JSON.parse(realRaw) as Record<string, unknown>;

  const dir = makeTempDir("movan-core-state-load-");
  const tempPath = join(dir, "project-state.json");
  writeFileSync(tempPath, realRaw, "utf-8");

  const loaded = loadState(tempPath) as unknown as ProjectState<MovanProjectExtension>;
  check("A) 실제 MOVAN project-state.json을 새 ProjectState 타입으로 정상 load", typeof loaded === "object" && loaded !== null);
  ...
  // MOVAN 전용 필드(MovanProjectExtension) — 하나도 유실되지 않아야 한다.
  const movanFields: (keyof MovanProjectExtension)[] = [
    "project",
    "phase10Allowed",
    "migrationsApplied",
    "migrationsImmutable",
    "devSupabaseCreated",
    "devSupabaseConnected",
    "microsoftConnected",
    "productionDeployAllowed",
  ];
  for (const field of movanFields) {
    check(
      `B) MOVAN 전용 필드 '${field}' 유실 없이 그대로 로드됨`,
      JSON.stringify(loaded[field]) === JSON.stringify(original[field])
    );
  }

  // 타입으로 아직 모델링하지 않은 자유 형식 서술 필드(phase20Note 등)도 런타임 객체에는
  // 그대로 남아있어야 한다(타입 캐스트는 런타임 값을 잘라내지 않는다).
  check(
    "B) 타입으로 모델링하지 않은 서술 필드(phase20Note)도 유실 없이 로드됨",
    (loaded as unknown as Record<string, unknown>).phase20Note === original.phase20Note
  );
}
...
// MOVAN 전용 필드를 하나도 갖지 않는다(TS가 그 사실 자체를 컴파일 시점에 강제한다 — 타입에
// 없는 필드를 넣으면 컴파일 에러, 타입에 있는 필드가 빠지면 역시 컴파일 에러).
// ---------------------------------------------------------------------------
...
function scenarioCoreOnlyFixtureDrivesNextTaskDecision(): void {
  ...
  check(
    "D) MOVAN 전용 필드가 fixture 객체에 하나도 없음(Object.keys에 MOVAN 필드 부재)",
    !("project" in fixtureNoTasksDone) &&
      !("phase10Allowed" in fixtureNoTasksDone) &&
      !("migrationsApplied" in fixtureNoTasksDone) &&
      !("microsoftConnected" in fixtureNoTasksDone)
  );
...
// MOVAN 필드도 아니고 CoreState 필드도 아닌, 이 테스트만을 위한 임의의 프로젝트 전용 필드
// (예: 가상의 BILLION 프로젝트가 쓸 법한 필드)를 하나 심어서 왕복 후에도 그대로 남는지
// 확인한다 — 실제 automation/config/project-state.json은 전혀 건드리지 않는다.
// ---------------------------------------------------------------------------
function scenarioUnknownProjectFieldSurvivesRoundTrip(): void {
  const dir = makeTempDir("movan-core-state-roundtrip-");
  const statePath = join(dir, "project-state.json");

  const UNKNOWN_FIELD_VALUE = { note: "BILLION-style 확장 필드", count: 42 };
  const initial = {
    // CoreState 필드
    currentTask: null,
    reviewCycle: 0,
    ...
    // 몰라도 유실시키면 안 된다.
    billionOnlyField: UNKNOWN_FIELD_VALUE,
  };
  writeFileSync(statePath, JSON.stringify(initial, null, 2) + "\n", "utf-8");

  const loaded = loadState(statePath) as ProjectState & { billionOnlyField?: unknown };
  check(
    "E) load 직후 알 수 없는 필드(billionOnlyField)가 그대로 존재",
    JSON.stringify(loaded.billionOnlyField) === JSON.stringify(UNKNOWN_FIELD_VALUE)
  );
  ...
  check(
    ...
    JSON.stringify(reloadedRaw.billionOnlyField) === JSON.stringify(UNKNOWN_FIELD_VALUE)
  );
...
function main(): void {
  ...
  try {
    scenarioLoadRealMovanStateAndCheckNoFieldLoss();
  ...
  console.log("\n=== core-state(CoreState/MOVAN 전용 상태 분리) 테스트 결과 ===");
